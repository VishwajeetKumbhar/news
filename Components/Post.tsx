...
export default async function Post({
    ...
    return (
        <div className='my-4 border-b border-b-300 py-8'>
            <div className='mb-4 '>
                {author ? (<>Posted by: <span className='font-bold'> {author} </span> on {formattedDate}</>
                ) : (
                    <>
            ...
            <div className='w-full h-72 relative'>
                {thumbnail ? (<Image src={thumbnail} alt={title} fill className='object-cover rounded-md object-center' />
                    ...
                    <Image src={"/thumb.png"} alt={title} fill className='object-cover rounded-md object-center' />
                )}
            </div>
            ...
            )}
            {
                isEditable && (
                    <div className='flex gap-3 font-bold py-2 px-4 rounded-md bg-slate-200 w-fit'>
                        <Link className='' href={`/edit-post/${id}`}>Edit</Link>
                        <DeleteButton id={id} />
                    </div>
                )
            }
